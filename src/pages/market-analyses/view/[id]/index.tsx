...
import { getMarketAnalysisById } from 'apiSdk/market-analyses';
import { MarketAnalysisInterface } from 'interfaces/market-analysis';

function MarketAnalysisViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<MarketAnalysisInterface>(
    () => (id ? `/market-analyses/${id}` : null),
    () =>
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('market_analysis', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/market-analyses/edit/${id}`} passHref legacyBehavior>
                    <Button
