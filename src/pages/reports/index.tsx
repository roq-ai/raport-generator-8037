...
type ColumnType = ColumnDef<ReportInterface, unknown>;
...
export function ReportListPage(props: ReportListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ReportInterface>>(
    () => `/reports?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ReportInterface) => {
