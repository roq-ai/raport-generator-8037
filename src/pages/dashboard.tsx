...
import { getIndustryTrends } from 'apiSdk/industry-trends';
import { IndustryTrendsInterface } from 'interfaces/industry-trends';
import { getEconomicIndicators } from 'apiSdk/economic-indicators';
import { EconomicIndicatorsInterface } from 'interfaces/economic-indicators';
import { getReports } from 'apiSdk/reports';
import { ReportInterface } from 'interfaces/report';

import { RegionListPage as ShowCaseList } from 'pages/regions';

function DashboardPage() {
  const { session } = useSession();

  const industryTrendsStat = useSWR<PaginatedInterface<IndustryTrendsInterface>>(
    () => `/industry-trends`,
    () => getIndustryTrends({ limit: 1, offset: 0 }),
  );

  const economicIndicatorsStat = useSWR<PaginatedInterface<EconomicIndicatorsInterface>>(
    () => `/economic-indicators`,
    () => getEconomicIndicators({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: industryTrendsStat?.data?.totalCount,
      isLoading: industryTrendsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Economic Indicators',
      result: economicIndicatorsStat?.data?.totalCount,
      isLoading: economicIndicatorsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Reports',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`regions`} passHref legacyBehavior>
                  <Button
